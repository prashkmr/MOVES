...
class netG(nn.Module):
    def __init__(self, args, nz=100, ngf=64, nc=3, base=4, ff=(1,8)):
        ...
        layers += [nn.ConvTranspose2d(ngf * 2, ngf * 2, (2,4), stride=2, padding=(0,1), bias=False)]
        layers += [nn.BatchNorm2d(ngf * 2)]
        layers += [nn.ReLU(True)]

        layers += [nn.ConvTranspose2d(ngf * 2, ngf, (2,4), stride=2, padding=(1,1), bias=False)]
...
class netD(nn.Module):
    def __init__(self, args, ndf=64, nc=2, nz=1, lf=(1,8)):
        ...
        layers  = []
        layers += [nn.Conv2d(nc, ndf, (2,4), 2, 1, bias=False)]
        layers += [nn.LeakyReLU(0.2, inplace=True)]
        layers += [nn.Conv2d(ndf, ndf * 2, (2,4), 2, 1, bias=False)]

        layers += [nn.LeakyReLU(0.2, inplace=True)]
        layers += [nn.Conv2d(ndf*2, ndf * 2, (2,3), 2, 1, bias=False)]

        layers += [nn.BatchNorm2d(ndf * 2)]
        layers += [nn.LeakyReLU(0.2, inplace=True)]
        layers += [nn.Conv2d(ndf * 2, ndf * 4, (2,3), 2, 1, bias=False)]

        layers += [nn.LeakyReLU(0.2, inplace=True)]
        layers += [nn.Conv2d(ndf * 4, ndf * 4, (2,3), 2, 1, bias=False)]
