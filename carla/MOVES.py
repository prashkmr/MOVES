...
parser.add_argument('--reload',             type=str,   default='',            help='size of the bottleneck dimension in the VAE, or the latent noise size in GAN')
...
def loadNP(index):
    ...
    for i in trange(index):
        ...
        static.append( np.load( DATA  + "lidar/d{}.npy".format(i) ) [:,:,5:45,:].astype('float32'))
        dynamic.append( np.load( DATA + "lidar/s{}.npy".format(i)  )[:,:,5:45,:].astype('float32'))
...
# writer = tf.SummaryWriter(log_dir=os.path.join(args.base_dir, 'TB'))
writes = 0
...
for epoch in range(epoch,1501):
    ...
    for i in npyList1:
        ...
        while iters < len(i):
            ...
            recloss = 0
            recloss = loss_fn(recon2[:,:,0:40,:], inputs[1].cuda()).mean(dim=0)
